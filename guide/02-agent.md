...
- **完成回传**：任务结束把结果**入队为一条通知消息**（`task-notification`，`later` 优先级），经统一命令队列回到主循环。**关键：提交轮从不为它空等**——默认它**留在队列、等主循环空闲后另起一个新提交轮**送达（除非模型主动调 `Sleep` 把它拉进当轮）。完整时序（提交轮 vs 工具轮、两个 drain 闸口、与用户输入的优先级关系）见**[《全景与主循环》](./00-overview.md)§3.1**；通知的 XML 原文格式与 ID 匹配见下 §4.1 ④。
